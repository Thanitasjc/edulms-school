...
export default function CheckoutPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response) => {
      clearCart();
      const skipped = response.data.skipped_course_ids?.length ?? 0;
      toast.success(
        skipped > 0
          ? `Enrolled ${response.data.purchased_count} course(s). ${skipped} already owned.`
          : `Successfully enrolled in ${response.data.purchased_count} course(s).`,
      );
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-12 lg:px-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-950 lg:col-span-7">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Demo checkout — no real payment gateway. Confirming will enroll you in the selected courses
            instantly (mock payment in THB).
          </p>
        ...
        <aside className="lg:col-span-5">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-950">
            ...
            >
              {mutation.isPending ? "Processing..." : "Confirm & Enroll"}
            </Button>
            <Link href="/cart" className="mt-3 block text-center text-sm text-blue-600 hover:underline">
              Back to cart
            </Link>
          </div>
        </aside>
